...
// ✅ FIX CRITIQUE : validerConsignation — si chargé a déjà validé (consigne_charge),
//    le process valide et le statut passe directement à 'consigne' (COMPLET)
// ✅ FIX : getDemandesADeconsigner inclut 'deconsignee' et 'cloturee'
// ✅ FIX : getDemandesAConsigner inclut 'consigne_charge'
// ✅ FIX DÉCONSIGNATION : validerDeconsignationFinale
//    - chargeDejaDecons = statut === 'deconsigne_charge' UNIQUEMENT
//    - Autorise 'deconsigne_charge' dans STATUTS_DECONS_OK
//    - Bloque si statut === 'deconsigne_process' (process a déjà validé)
    ...
    const [rows] = await db.query(
      `SELECT d.*,
       ...
       WHERE d.statut IN (
         ...
         'consigne', 'consigne_process', 'consigne_charge',
...
const getDemandeDeconsignationDetail = async (req, res) => {
  try {
    ...
    console.error('getDemandeDeconsignationDetail (process) error:', err);
    return error(res, 'Erreur serveur', 500);
  }
};
...
const scannerCadenasDeconsignation = async (req, res) => {
  try {
    ...
    const [demandes] = await db.query(
      `SELECT id, statut, deconsignation_demandee FROM demandes_consignation
       WHERE id = ? AND deconsignation_demandee = 1`, [id]
    );
    if (!demandes.length) return error(res, 'Demande introuvable ou déconsignation non demandée', 404);
    ...
    if (!cadenas_ok) {
      return error(res,
        `Cadenas incorrect. Attendu: ${point.cadenas_consigne} — Scanné: ${cadenas_scanne}`, 400
      );
    }
    ...
    await db.query(
      `INSERT INTO deconsignations (point_id, numero_cadenas, deconsigne_par, date_deconsigne)
       VALUES (?, ?, ?, NOW())`,
      [point_id, cadenas_scanne, process_id]
    );
    ...
    if (planRows.length > 0) {
      ...
      const [totalRows] = await db.query(
        `SELECT COUNT(*) AS total FROM points_consignation WHERE plan_id = ? AND charge_type = 'process'`, [planId]
      );
      const [deconsFaits] = await db.query(
        `SELECT COUNT(*) AS fait FROM deconsignations dc
         JOIN points_consignation pc ON pc.id = dc.point_id
         WHERE pc.plan_id = ? AND pc.charge_type = 'process'`, [planId]
      );
      tousDeconsignes = totalRows[0].total > 0 && deconsFaits[0].fait >= totalRows[0].total;
    }

    return success(res, {
      point_id, cadenas_scanne, cadenas_ok: true, tous_valides: tousDeconsignes,
    }, 'Cadenas process déconsigné avec succès');
  } catch (err) {
    console.error('scannerCadenasDeconsignation (process) error:', err);
    return error(res, 'Erreur serveur', 500);
  }
};
...
const demarrerConsignation = async (req, res) => {
  try {
    ...
    if (['en_cours', 'consigne_process', 'consigne_charge'].includes(statut)) {
      return success(res, null, `Consignation déjà en cours (${statut})`);
    }
    await db.query(
...
const validerConsignation = async (req, res) => {
  try {
    ...
    const statutsAutorisesProcess = ['en_attente', 'en_cours', 'validee', 'consigne_charge'];
    if (!statutsAutorisesProcess.includes(demande.statut)) {
      return error(res, `Statut invalide pour valider : ${demande.statut}`, 400);
    }
    ...
    const remarques = nouveauStatut === 'consigne'
      ? chargeDejaValide
        ? 'Consignation complète — Process validé EN SECOND (chargé déjà validé) — PDF unifié final'
    ...
    return success(res, {
      ...
      charge_deja_valide: chargeDejaValide,
...
// ════════════════════════════════════════════════════════════════
// ✅ validerDeconsignationFinale — CORRIGÉ
//
// RÈGLE :
//   - hasElec : vérifie points réels (charge_type='electricien') ET types_intervenants
//   - chargeDejaDecons = statut === 'deconsigne_charge' UNIQUEMENT
//     (NE PAS utiliser date_validation_charge = concerne la CONSIGNATION)
//   - Si process a déjà validé (deconsigne_process) → erreur double validation
//   - Si chargé a validé en premier (deconsigne_charge) → process peut encore valider
//
// STATUTS :
//   Pas d'électrique                    → 'deconsignee' (process seul suffit)
//   électrique existe + chargé déjà décons → 'deconsignee' (les deux ont validé)
//   électrique existe + chargé pas encore  → 'deconsigne_process' (en attente chargé)
// ════════════════════════════════════════════════════════════════
const validerDeconsignationFinale = async (req, res) => {
  try {
    ...
    // ✅ FIX : Bloquer si le process a DÉJÀ validé sa déconsignation
    if (demande.statut === 'deconsigne_process') {
      return error(res, 'Vous avez déjà validé la déconsignation process', 400);
    }

    // ✅ FIX : Autoriser 'deconsigne_charge' (chargé a validé en premier → process peut encore valider)
    const STATUTS_DECONS_OK = [
      'deconsigne_genie_civil', 'deconsigne_mecanique', 'deconsigne_electrique',
      'deconsigne_gc', 'deconsigne_mec', 'deconsigne_elec',
      'deconsigne_intervent', 'deconsigne_charge',
      'consigne', 'consigne_process', 'consigne_charge',
    ];
    ...
    if (plan) {
      const [totalProcess] = await db.query(
        `SELECT COUNT(*) AS total FROM points_consignation WHERE plan_id = ? AND charge_type = 'process'`, [plan.id]
      );
      const [deconsFaits] = await db.query(
        `SELECT COUNT(*) AS fait FROM deconsignations dc
         JOIN points_consignation pc ON pc.id = dc.point_id
         WHERE pc.plan_id = ? AND pc.charge_type = 'process'`, [plan.id]
      );
      if (totalProcess[0].total > 0 && deconsFaits[0].fait < totalProcess[0].total) {
        const restants = totalProcess[0].total - deconsFaits[0].fait;
        return error(res,
          `${restants} cadenas process n'ont pas encore été scannés. Veuillez scanner tous les cadenas avant de valider.`,
          400
        );
      }
    }
    ...
    if (plan) {
      const [pts] = await db.query(
        `SELECT pc.*,
                ...
                CONVERT_TZ(dc.date_deconsigne,'+00:00','+01:00') AS date_decons,
    ...
    // ✅ FIX LOGIQUE STATUT :
    // Vérifier si des points électriques existent dans le plan
    const pointsElec = plan ? points.filter(p => p.charge_type === 'electricien' || !p.charge_type) : [];
    const hasElec = pointsElec.length > 0 ||
                    demande.types_intervenants.includes('electrique') ||
                    demande.types_intervenants.includes('electricien');

    // ✅ FIX CRITIQUE : chargeDejaDecons = uniquement statut === 'deconsigne_charge'
    // NE PAS utiliser date_validation_charge (c'est la date de CONSIGNATION, pas déconsignation)
    const chargeDejaDecons = demande.statut === 'deconsigne_charge';

    let nouveauStatut;
    if (!hasElec) {
      nouveauStatut = 'deconsignee';        // Pas d'électrique → process seul suffit
    } else if (chargeDejaDecons) {
      nouveauStatut = 'deconsignee';        // Chargé a déjà déconsigné → les deux ont validé
    } else {
      nouveauStatut = 'deconsigne_process'; // Chargé n'a pas encore déconsigné → en attente
    }

    const dateCol = nouveauStatut === 'deconsignee' ? ', date_deconsignation=NOW()' : '';
    await db.query(
      `UPDATE demandes_consignation SET statut=?, pdf_path_final=?, updated_at=NOW() ${dateCol} WHERE id=?`,
      [nouveauStatut, pdfRelPath, id]
    );

    await db.query(
      `UPDATE dossiers_archives SET pdf_path=?, cloture_par=?, date_cloture=NOW(), remarques=? WHERE demande_id=?`,
      [pdfRelPath, process_id, `PDF déconsignation process — ${nouveauStatut}`, id]
    );
    ...
    if (nouveauStatut === 'deconsignee') {
      ...
      // Process valide en premier → notifier le chargé
      await envoyerNotification(demande.agent_id_val, '⚙️ Déconsignation process validée',
        ...
        'deconsignation', `demande/${id}`);
      await envoyerPushNotification([demande.agent_id_val], '⚙️ Déconsignation process effectuée',
        `${demande.numero_ordre} — vannes process déconsignées. En attente chargé.`,
        { demande_id: id, statut: 'deconsigne_process' });
      ...
      if (chargeIds.length > 0) {
        await envoyerNotificationMultiple(chargeIds, '🔔 Déconsignation électrique requise',
          `Le process a déconsigné ses points sur ${demande.tag}. C'est votre tour de déconsigner les cadenas électriques.`,
          'deconsignation', `demande/${id}`);
    ...
    if (!peutVoir) {
      if (demande.statut === 'consigne_process') {
        return res.status(403).json({
          ...
          statut:  demande.statut, besoin: 'validation_charge',
        });
      }
      return res.status(403).json({
        message: 'Vous devez valider la consignation process avant de pouvoir accéder au PDF',
        statut:  demande.statut, besoin: 'validation_process',
      });
    }
  ...
  try {
    ...
    const [points] = await db.query(
      `SELECT pc.*, ec.numero_cadenas AS cadenas_pose
       ...
       WHERE pc.id = ? AND pc.charge_type = 'process' LIMIT 1`, [pointId]
    );
    ...
    if (dejaTrace.length) {
      ...
      await db.query(
        `INSERT INTO deconsignations (point_id, numero_cadenas, deconsigne_par, date_deconsigne) VALUES (?, ?, ?, NOW())`,
