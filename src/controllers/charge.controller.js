...
// ✅ FIX CRITIQUE : getDemandesAConsigner inclut 'consigne_process'
// ✅ FIX : getDemandesADeconsigner inclut 'deconsignee' et 'cloturee'
// ✅ FIX : validerConsignation — si process a déjà validé (consigne_process), statut → 'consigne'
// ✅ FIX DÉCONSIGNATION : validerDeconsignationFinale
//    - processDejaDecons = statut === 'deconsigne_process' UNIQUEMENT
//    - Autorise 'deconsigne_process' dans STATUTS_DECONS_OK
//    - Bloque si statut === 'deconsigne_charge' (chargé a déjà validé)
...
const demarrerConsignation = async (req, res) => {
  try {
    ...
    if (!statutsAutorisesCharge.includes(current.statut)) {
      return success(res, {
        date_validation_charge:  current.date_validation_charge,
        date_validation_process: current.date_validation_process,
      }, `Statut actuel : ${current.statut}`);
    }
    ...
    await db.query(
      `UPDATE demandes_consignation
       SET statut=?, charge_id=?, updated_at=NOW()
       WHERE id=?`,
      [newStatut, charge_id, id]
    );
...
const validerConsignation = async (req, res) => {
  try {
    ...
    const statutsAutorisesCharge = ['en_attente', 'en_cours', 'validee', 'consigne_process'];
    if (!statutsAutorisesCharge.includes(demande.statut)) {
    ...
    if (pointsElec.length > 0) {
      await db.query(
        `UPDATE points_consignation SET statut='verifie'
         WHERE plan_id=? AND charge_type IN ('electricien','') AND statut='consigne'`,
        [plan ? plan.id : 0]
      );
    }
    ...
    const remarques = nouveauStatut === 'consigne'
      ? processDejaValide
        ? 'Consignation complète — Chargé validé EN SECOND (process déjà validé) — PDF unifié final'
    ...
    return success(res, {
      ...
      process_deja_valide: processDejaValide,
...
//
// RÈGLE :
//   - hasProcess : vérifie points réels (charge_type='process') ET types_intervenants
//   - processDejaDecons = statut === 'deconsigne_process' UNIQUEMENT
//     (NE PAS utiliser date_validation_process = concerne la CONSIGNATION)
//   - Si chargé a déjà validé (deconsigne_charge) → erreur double validation
//   - Si process a validé en premier (deconsigne_process) → chargé peut encore valider
//
// STATUTS :
//   Pas de process                    → 'deconsignee' (chargé seul suffit)
//   process existe + process déjà décons → 'deconsignee' (les deux ont validé)
//   process existe + process pas encore  → 'deconsigne_charge' (en attente process)
// ════════════════════════════════════════════════════════════════
const validerDeconsignationFinale = async (req, res) => {
  try {
    ...
    // ✅ FIX : Bloquer si le chargé a DÉJÀ validé sa déconsignation
    if (demande.statut === 'deconsigne_charge') {
      return error(res, 'Vous avez déjà validé la déconsignation électrique', 400);
    }

    // ✅ FIX : Autoriser 'deconsigne_process' (process a validé en premier → chargé peut encore valider)
    ...
    if (demande.statut === 'deconsigne_process') {
      const [procRows] = await db.query(
        `SELECT DISTINCT u.prenom, u.nom
         ...
         JOIN points_consignation pc ON pc.id = dc.point_id
         JOIN plans_consignation pl  ON pl.id = pc.plan_id
         JOIN users u ON u.id = dc.deconsigne_par
    ...
    // ✅ FIX LOGIQUE STATUT :
    // Vérifier si des points process existent dans le plan
    const pointsProcess = plan ? points.filter(p => p.charge_type === 'process') : [];
    const hasProcess = pointsProcess.length > 0 ||
                       demande.types_intervenants.includes('process');

    // ✅ FIX CRITIQUE : processDejaDecons = uniquement statut === 'deconsigne_process'
    // NE PAS utiliser date_validation_process (c'est la date de CONSIGNATION)
    const processDejaDecons = demande.statut === 'deconsigne_process';

    let nouveauStatut;
    if (!hasProcess) {
      nouveauStatut = 'deconsignee';       // Pas de process → chargé seul suffit
    } else if (processDejaDecons) {
      nouveauStatut = 'deconsignee';       // Process a déjà déconsigné → les deux ont validé
    } else {
      nouveauStatut = 'deconsigne_charge'; // Process n'a pas encore déconsigné → en attente
    }
    ...
    if (nouveauStatut === 'deconsignee') {
      ...
      // Chargé valide en premier → notifier le process
      await envoyerNotification(demande.agent_id_val, '⚡ Déconsignation électrique validée',
        ...
        'deconsignation', `demande/${id}`);
      await envoyerPushNotification([demande.agent_id_val], '⚡ Déconsignation électrique effectuée',
        `${demande.numero_ordre} — points électriques déconsignés. En attente process.`,
        { demande_id: id, statut: 'deconsigne_charge' });
